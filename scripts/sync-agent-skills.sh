...
for skill in "${SKILLS[@]}"; do
  ...
  if [ -n "$when" ]; then
    [ "${#when}" -le 120 ] || { echo "ERROR: $skill: derived trigger is ${#when} chars (max 120) — shorten description\x27s first sentence" >&2; exit 1; }
...
if [ "${#rules_rows[@]}" -gt 0 ]; then
  {
    ...
    echo
    echo "# Rule index — which skill to load, and when"
    echo
  ...
  echo "Wrote $(basename "$rules_file") (${#rules_rows[@]} routes)"
else
  rm -f "$rules_file"
fi
