...
lock_entries=()
rules_skills=()   # skills shipping an agent-rules.md — assembled into .agents/AGENT_RULES.md below
...
for skill in "${SKILLS[@]}"; do
  ...
  [ -f "$src_skill_dir/agent-rules.md" ] && rules_skills+=("$skill")
...
if [ "${#rules_skills[@]}" -gt 0 ]; then
  mapfile -t rules_skills < <(printf '%s\n' "${rules_skills[@]}" | LC_ALL=C sort)
fi

# --- Write (or remove) .agents/AGENT_RULES.md: the concatenated agent-rules.md of every vendored skill
# that ships one — a trigger-indexed set of standing rules for your agent's ALWAYS-LOADED instructions.
# Import it once: add `@.agents/AGENT_RULES.md` to CLAUDE.md (see README.md "Agent rules"). Regenerated
# every sync from the CURRENTLY vendored SKILLS set, so dropping a skill (or its agent-rules.md upstream)
# updates this file too — no separate prune step needed.
rules_file="$repo_root/.agents/AGENT_RULES.md"
if [ "${#rules_skills[@]}" -gt 0 ]; then
  {
    echo "<!-- Generated by scripts/sync-agent-skills.sh from vendored skills' agent-rules.md — do not"
    echo "     edit by hand. Vendored from $PLAYBOOK_REPO @ $resolved_sha. -->"
    echo
    echo "# Agent rules"
    echo
    echo "Standing rules, indexed by the situation that triggers them. Each block names the skill that owns"
    echo "the full reasoning — **load that skill** when its trigger fires and you need the detail."
    echo "When a rule here and its source skill disagree, **the skill wins and this file is stale**."
    for sk in "${rules_skills[@]}"; do
      echo
      cat "$vendor_dir/$sk/agent-rules.md"
    done
  } > "$rules_file"
  echo "Wrote $(basename "$rules_file") (${#rules_skills[@]} rule block(s))"
else
  rm -f "$rules_file"
fi
# Supersedes the earlier GLOBAL_HINTS.md (one-line-per-skill frontmatter hints). Remove a stale one so a
# consumer that still imports it doesn't silently keep loading rules that are no longer regenerated.
rm -f "$repo_root/.agents/GLOBAL_HINTS.md"
